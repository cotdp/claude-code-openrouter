...
usage_segment() {
  ...
  [ -r "$CACHE" ] || return 0
  # label<TAB>percent per active quota; limits[] preferred, legacy keys as fallback
  jq -r '
    (if (.limits | type) == "array" and (.limits | length) > 0 then
       [.limits[] | select(.percent != null) | {
          label: (if .kind == "session" then "5h"
                  ...
                  elif .kind == "weekly_scoped" then (.scope.model.display_name // "scoped")
                  else .kind end),
          pct: .percent}]
     else
    ...
    | .[] | "\(.label)\t\(.pct)"
  ' "$CACHE" 2>/dev/null | while IFS="$(printf '\t')" read -r label pct; do
    p="${pct%%.*}"
